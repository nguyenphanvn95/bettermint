"use strict";
const DefaultExtensionOptions2 = {
    depth: 3,
    elo: 1500,
    skill_level: 10,
    multipv: 3,
    threads: 10,
    auto_move_time: 5000,
    auto_move_time_random: 10000,
    auto_move_time_random_div: 10,
    auto_move_time_random_multi: 1000,
    max_legit_auto_move_depth: 5,
    best_move_chance: 30,
    matefindervalue: 3,
    limit_strength: false,
    highmatechance: false,
    random_best_move: false,
    legit_auto_move: false,
    show_hints: true,
    text_to_speech: false,
    move_analysis: true,
    depth_bar: true,
    evaluation_bar: true,
    use_nnue: false
};

// NOTE (2026): chess.com enforces a restrictive CSP that can block DOM-based
// <script src="chrome-extension://..."> injection.
// In MV3, the recommended way is chrome.scripting.executeScript({world:"MAIN"})
// via the service worker.
function requestMainWorldInjection() {
    try {
        chrome.runtime.sendMessage({ type: "BETTERMINT_INJECT" });
    } catch (e) {
        // If messaging fails, we silently ignore to avoid breaking chess.com.
    }
}
chrome.runtime.onMessage.addListener(function (request, sender, sendResponse) {
    // pass the event to injected script
    if (request.data !== 'popout') {
        window.dispatchEvent(new CustomEvent("BetterMintUpdateOptions", {
            detail: request.data
        }));
    } else if (request.data == 'popout') {
        window.postMessage('popout');
    }
});
window.addEventListener("BetterMintGetOptions", function (evt) {
    chrome.storage.sync.get(DefaultExtensionOptions2, function (opts) {
        let request = evt.detail;
        let response = {
            requestId: request.id,
            data: opts
        };
        window.dispatchEvent(new CustomEvent("BetterMintSendOptions", {
            detail: response
        }));
    });
});
requestMainWorldInjection(); // Injects bettermint.js in MAIN world